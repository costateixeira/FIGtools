...
REM        for example https://github.com/WorldHealthOrganization/smart-ips-pilgrimage.git
REM 2. Checks out the branch "gh-pages" and copies the content
REM    from the folder "sitepreview\<folder1>" (ignoring history)
REM        for example sitepreview\ips-pilgrimage,
REM    and also copies these additional files:
REM        sitepreview\package-feed.xml
REM        sitepreview\package-registry.json
REM        sitepreview\publication-feed.xml
REM    into a temporary folder.
REM 3. Clones the target repository from:
REM    https://github.com/<org2>/<repo2>.git
REM        for example https://github.com/medigree/smart-html.git
...
REM    with the base set to %TGT_BRANCH%.
REM ==================================================================

REM ================ CONFIGURATION ===================
REM Source repository settings
set "SRC_ORG=SourceOrg"
set "SRC_REPO=source-repo"
set "SRC_BRANCH=gh-pages"
set "SRC_FOLDER=sitepreview\my-ig"

REM Target folder inside the repository
set "TGT_FOLDER=my-ig"  

REM Target repository settings
set "TGT_ORG=myorg"
set "TGT_REPO=myrepo"
set "TGT_BRANCH=main"
set "NEW_BRANCH=newbranch"


REM PR title and body (adjust as needed)
set "PR_TITLE=Import content to target folder"
set "PR_BODY=This PR imports content from %SRC_REPO% (%SRC_BRANCH%/%SRC_FOLDER%) and additional package files into the '%TGT_FOLDER%' folder of %TGT_REPO%."
...
set "rc=%ERRORLEVEL%"
if %rc% GEQ 8 (
    echo Error copying files from the source repository folder.
    pause
    exit /b 1
)

echo Copying additional package files to %TEMP_CONTENT%...
copy "%SRC_REPO%\sitepreview\package-feed.xml" "%TEMP_CONTENT%\"
if errorlevel 1 (
    echo Error copying package-feed.xml.
    pause
    exit /b 1
)
copy "%SRC_REPO%\sitepreview\package-registry.json" "%TEMP_CONTENT%\"
if errorlevel 1 (
    echo Error copying package-registry.json.
    pause
    exit /b 1
)
copy "%SRC_REPO%\sitepreview\publication-feed.xml" "%TEMP_CONTENT%\"
if errorlevel 1 (
    echo Error copying publication-feed.xml.
    pause
    exit /b 1
)
...
git commit -m "Import %SRC_FOLDER% and additional package files to %TGT_FOLDER%"
if errorlevel 1 (
